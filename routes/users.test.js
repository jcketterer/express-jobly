...
/************************** POST job application tests */

describe('POST /users/:username/jobs/:id', function () {
  test('works for admin', async function () {
    const resp = await request(app).post(`/users/u1/jobs/${testJobId[1]}`).set('authorization', `Bearer ${adminToken}`)
    expect(resp.body).toEqual({ applied: testJobId[1] })
  })

  test('works for same user', async function () {
    const resp = await request(app).post(`/users/u1/jobs/${testJobId[1]}`).set('authorization', `Bearer ${u1Token}`)
    expect(resp.body).toEqual({ applied: testJobId[1] })
  })

  test('unauth for others', async function () {
    const resp = await request(app).post(`/users/u1/jobs/${testJobId[1]}`).set('authorization', `Bearer ${u2Token}`)
    expect(resp.statusCode).toEqual(401)
  })

  test('unauth for anon', async function () {
    const resp = await request(app).post(`/users/u1/jobs/${testJobId[1]}`)
    expect(resp.statusCode).toEqual(401)
  })

  test('not found for no such username', async function () {
    const resp = await request(app)
      .post(`/users/nope/jobs/${testJobId[1]}`)
      .set('authorization', `Bearer ${adminToken}`)
    expect(resp.statusCode).toEqual(404)
  })
